...
class InputUnit(nn.Module):    
    ...
    def forward(self, video, question, question_len):
        ...
        attn_w = self.softm(self.temp_attn)
        v_sz = video.size()
        video = attn_w * video.permute(1,0,2,3,4).reshape(v_sz[1],-1)
        video = video.reshape(v_sz[1],v_sz[0],v_sz[2],v_sz[3],v_sz[4]).permute(1,0,2,3,4)
        image = video.sum(dim=1)

...
class ReadUnit_Frame(nn.Module):
    ...
    def forward(self, memory, know, control, memDpMask=None):
        ...
        Args:
            ...
            know: representation of the knowledge base (image).
                [batchSize, kbSize (num_slots)), memDim]
...
class ReadUnit(nn.Module):
    def __init__(self, module_dim, dropout=0.15):
        super().__init__()
        self.module_dim = module_dim
        self.read_frame = ReadUnit_Frame(module_dim, dropout)
        self.frame_lstm = nn.LSTM(module_dim, module_dim, batch_first=True, bidirectional=False)
    ...
    def forward(self, memory, know, control, memDpMask=None):
        ...
        Args:
            memory: the cell's memory state
                [batchSize, memDim]
            know: representation of the knowledge base (image).
                [batchSize, num_frames, kbSize (num_slots)), memDim]
            control: the cell's control state
                [batchSize, ctrlDim]
            memDpMask: variational dropout mask (if used)
                [batchSize, memDim]
...
class InputUnit(nn.Module):    
    ...
    def monet_v_p(self, video):
        ...
        frame_encs = frame_encs.view(cb_sz[0], cb_sz[1], cb_sz[2], self.dim)
        return frame_encs
    ...
    def __init__(self, cfg, vocab_size, wordvec_dim=300, rnn_dim=512, module_dim=512, bidirectional=True):
        ...
        #                           nn.ELU())
        self.res_proj = nn.Linear(16, module_dim)
